...
✅ Частично реализован Epic AUT-4 (AI-чат): AUT-17 (чат-инфраструктура), AUT-19 (Режим 1: FaultHelp — многошаговая диагностика + DiagnosticResultCard на фронтенде), AUT-20 (Режим 2: WorkClarification — одношаговый анализ работ + рыночные бенчмарки), AUT-21 (Режим 3: PartnerAdvice — поиск партнёров: LLM-классификация категории → PartnerSearchService (own DB + Google Places) → LLM-форматирование + PartnerAdviceResultCard на фронтенде)
...
### Ревью после каждого блока работы (ОБЯЗАТЕЛЬНО)

После написания или изменения кода — **обязательно** провести ревью через соответствующий скилл:

- **Backend (.NET/C#)** — вызвать `dotnet-code-reviewer` на весь изменённый код.
- **Frontend (Next.js/TypeScript)** — вызвать `nextjs-code-reviewer` (или ближайший фронтенд-скилл) на изменённые компоненты/страницы.
- Если в рамках одной задачи менялись и бэк, и фронт — ревью проводится для каждой части отдельно.

Ревью проводится **до коммита**. Все найденные must-fix и should-fix проблемы исправляются немедленно.

### Сверка с исходным запросом (ОБЯЗАТЕЛЬНО в конце работы)

После завершения всей работы (и ревью) — явно сверить выполненное с исходным запросом пользователя:

1. Перечитать оригинальный запрос.
2. Пройтись по каждому пункту требований / acceptance criteria.
3. Для каждого пункта однозначно указать: ✅ выполнено / ❌ не выполнено / ⚠️ выполнено частично.
4. Если есть невыполненные или частично выполненные пункты — сообщить пользователю и либо доделать, либо явно объяснить причину пропуска.
