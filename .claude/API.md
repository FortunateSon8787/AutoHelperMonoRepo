...
Постранично возвращает чат-сессии текущего клиента (без тел сообщений, только count). Упорядочены по убыванию даты создания. Soft-deleted чаты (`IsDeleted = true`) не возвращаются — ни удалённые вручную, ни автоматически (при невалидном запросе).
...
- `initialAssistantReply` — для всех режимов содержит текстовый ответ ассистента; для off-topic запросов — rejection-сообщение
...
- `chatStatus` — итоговый статус чата: `Active` | `Completed`
- `wasValid` — `false` если запрос был off-topic; в этом случае чат автоматически soft-delete'ится (`IsDeleted = true`) и больше не возвращается в `GET /api/chats` и `GET /api/chats/{id}`

**Поведение при невалидном запросе:**
- `wasValid = false` означает, что пользовательский запрос был off-topic для выбранного режима
- Чат автоматически переводится в `Completed` и помечается `IsDeleted = true` в той же транзакции
- Ответ с `wasValid = false` всё равно возвращается клиенту (с rejection-сообщением в `initialAssistantReply`)
- Фронтенд показывает невалидный чат в окне до навигации, но не добавляет его в список «Недавних бесед»
...
- `wasValid: false` — запрос был off-topic; вернулась стандартная заглушка, квота не уменьшена; чат автоматически soft-delete'ится и больше не возвращается в `GET /api/chats`
...
- **Автоматический soft-delete при невалидном запросе**: при `wasValid = false` чат немедленно переводится в `Completed` и помечается `IsDeleted = true` в той же транзакции; исключается из `GET /api/chats` и `GET /api/chats/{id}/messages`; фронтенд показывает его в окне чата до навигации, но не отображает в списке «Недавних бесед»
